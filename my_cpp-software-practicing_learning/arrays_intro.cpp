...
int main() {
    ...
    int intarray[n]; // thankfully in Cpp array size can be dynamic
    int i;
    for ( i = 0; i <= n ; i++) {
        cin >> intarray[i];     // to store the members into the array
    };
    
    for ( i = n-1; i >= 0 ; i--) {
        cout << intarray[i] << " "; //to print it out in reverse order
    }
    cout << endl;
