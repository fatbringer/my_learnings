...
int main() {
    ...
    for ( i = 0 ; i < n ; i++) {
        ...
        for ( j = 0; j < k; j++) {
           int tempval;
           cin >> tempval; // you will need to cin to a temporary variable
           vectorOfArrays[i].push_back(tempval); // put in the kth members using the push_back method of the vector type
       }
    }
    ...
    for ( i = 0; i < q; i++){
        int arridx, membidx; 
        cin >> arridx >> membidx;
        cout << vectorOfArrays[arridx].at(membidx) << endl; // use the at method to dig out the correct member
